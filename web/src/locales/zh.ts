export default {
  translation: {
    common: {
      ...
      previewNotAvailable: '该文件不支持预览',
      previewNotAvailableForType: '{{type}} 文件不支持预览',
      previewNotAvailableTip: '请下载文件后在本地应用中打开。',
      figure: '图',
      close: '关闭',
      preview: '预览',
      move: '移动',
...
NER：使用 spaCy NER 和基于规则的关键词提取来抽取 Entities 和 Relations ，无需 LLM 参与提取过程，速度快且资源消耗低`,
    ...
    chunk: {
      ...
      emptyNotParsed: '该文档尚未解析',
      emptyNotParsedTip: '请在数据集文件列表中启动解析以生成切片。',
      emptyParsing: '解析进行中（{{progress}}%）',
      emptyParsingTip: '解析完成后切片会显示在这里。',
      emptyParseFailed: '解析失败，未生成任何切片',
      emptyParseCancelled: '解析已取消，未生成任何切片',
      emptyNoChunksProduced: '解析已完成，但未生成任何切片',
      emptyNoChunksProducedTip:
        '解析器未从该文件中提取到内容。未做 OCR 的扫描版 PDF、空文件，以及所选切片方法无法读取的文件类型都会出现这种情况。',
      parsingLog: '解析日志',
      representation: '表征',
      representationKinds: {
