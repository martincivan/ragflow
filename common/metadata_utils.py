...
async def apply_meta_data_scope(
    ...
    def _apply_hard(conditions: list[dict], logic: str, manual: bool) -> bool:
        ...
        if active_keys is not None and cm.is_chunk_filterable(conditions, active_keys):
            try:
                ...
                return True
        scope.doc_ids = _constrain(_run_metadata_filter(conditions, logic))
    ...
    if current_metas is None:
        # The value space came back incomplete: any filter built from it could
        # exclude matching documents, so run the search unscoped instead.
        scope.doc_ids = None
        return scope
