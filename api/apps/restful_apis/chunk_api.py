...
def _set_dataset_tokenizer_language(dataset_id):
    """Point the shared tokenizer at the dataset's language before manual
    chunk tokenization. Returns an error response when the dataset lookup
    fails, else None. set_language() logs the resolved language."""
    from rag.nlp import rag_tokenizer

    ok, kb = KnowledgebaseService.get_by_id(dataset_id)
    if not ok:
        return get_error_data_result(message=f"Dataset {dataset_id} not found.")
    rag_tokenizer.tokenizer.set_language(kb.language or "English")
    return None
...
async def retrieval_test(tenant_id, dataset_id=None):
    ...
    if meta_data_filter:
        ...
        if chunk_meta is not None and chunk_metadata.is_chunk_filterable(conditions, chunk_meta.fields) and not doc_ids:
    ...
    try:
        ...
        # loud rather than leaving the caller to guess. A chunk-field filter
        # also leaves `doc_ids` empty, but there the filter did run.
        meta_filter_summary = _meta_filter_summary(len([doc_id for doc_id in doc_ids if doc_id != "-999"]) if doc_ids else 0, doc_ids is None and not chunk_meta_filter)
...
async def add_chunk(tenant_id, dataset_id, document_id):
    ...
    err = _set_dataset_tokenizer_language(dataset_id)
    if err is not None:
        return err
